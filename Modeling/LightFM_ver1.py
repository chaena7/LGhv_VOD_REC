...
class LightFM_Model:
    ...
    # 평점 데이터 - self.rating_type 에 따라 다르게 설정
    def create_rating_df(self, vod_log, cont_log):
        ...
        if self.rating_type == 1:
            ...
            # rating_type = 2일때, 시청기록 + 클릭수 + 최근성 (합으로 연결)
            log_rating = log.copy()
            ...
            # rating_type = 3일때, 시청기록 * 클릭수 * 최근성 (곱으로 연결)
            log_rating = log.copy()
            log_rating.loc[log_rating[log_rating['watch_cnt'] > 0].index, '시청여부'] = 1
            scaler = MinMaxScaler()
            ...
            # rating_type = 4일때, 시청데이터를 PCA 차원축소한 데이터
            log_rating = log.copy()
            log_rating.loc[log_rating[log_rating['watch_cnt'] > 0].index, '시청여부'] = 1
